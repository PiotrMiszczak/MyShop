...
const addItem = (_id, qty) => async (dispatch, getState) => {
  try {
    ...
    Cookie.set("cartItems", JSON.stringify(cartItems)); //  Can be Local Storage
  } catch (error) {
    alert("404");
  }
};
...
const deleteProduct = (product) => async (dispatch, getState) => {
  try {
    ...
    const deletedProduct = await axios.delete(`/api/products/${product._id}`, {
      headers: { Authorization: `bearer ${userInfo.token}` },
    });
